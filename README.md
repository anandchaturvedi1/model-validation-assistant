# 🧠 Model Validation Assistant

A smart assistant that helps risk modelers and validators **review, explain, and document financial models** using **LLMs, LangChain, and Retrieval-Augmented Generation (RAG)**.


---

# 🚀 Usage
Currently live on Hugging Face Spaces:
https://huggingface.co/spaces/achaturvedi1/model-validation-assistant (To begin, you can use the ‘Restart this Space’ button)

Example model docs that can be used as input:
https://github.com/anandchaturvedi1/model-validation-assistant/tree/main/examples

Sample Output 1:

<img width="1339" height="648" alt="image" src="https://github.com/user-attachments/assets/65a1dd09-fdda-4e7d-9777-2d7c3699ac94" />

Sample Output 2:

<img width="1339" height="647" alt="image" src="https://github.com/user-attachments/assets/61fa255a-3b46-47d5-9942-e18a47c260f5" />


---

## 🚀 Features
- Parse model documentation (Txt, Word, PDF, SAS, Python)
- Summarize assumptions, inputs, and results
- Explain regulatory or statistical terms
- Compare model versions and highlight validation issues

---

## 🧩 Tech Stack
- **LangChain**
- **FAISS** for retrieval
- **Hugging Face Transformers** as LLM backend
- **Gradio (deployed on Hugging Face Spaces)** for automation

---

## 🧩 Current Status:
⚠️ Toy Model – Proof of Concept

